...
function dimensionChange(e,_isWidth) {
    ...
    var inImg2img = gradioApp().querySelector(".tabs button.bg-white.rounded-t-lg").innerText === I18N("img2img");
    if(!inImg2img) return;
    ...
    var img;
    if(img2imgMode==I18N("img2img")) {
        img = gradioApp().querySelector('div[data-testid=image] img');
    }else if(img2imgMode==I18N("Inpaint")) {
...
onLoad(function() {
    ...
    gradioApp().querySelectorAll("input").forEach(function(e){
        ...
        if(text&&(text=text.innerText)) {
            ...
            e.addEventListener('input', function(e){dimensionChange(e,text===I18N("Width"))});
        }
    });
});
