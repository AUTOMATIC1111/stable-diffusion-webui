...
def draw_xyz_grid(
    ...
    if draw_grid:
        ...
        for i in range(z_count):
            ...
            processed_result.all_seeds.insert(
                i, processed_result.all_seeds[start_index]
            )
            processed_result.infotexts.insert(
                i, processed_result.infotexts[start_index]
            )
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with InputAccordion(
            True, label="Draw grid", elem_id=self.elem_id("draw_grid")
        ) as draw_grid:
            with gr.Row():
