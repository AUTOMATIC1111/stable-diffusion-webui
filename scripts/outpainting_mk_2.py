...
def get_matched_noise(_np_src_image, np_mask_rgb, noise_q=1, color_variation=0.05):
    def _fft2(data):
        ...
        return np.fft.ifftshift(out_fft)

    def _ifft2(data):
        out_ifft = np.fft.ifft2(np.fft.fftshift(data), norm="ortho")
        return np.fft.ifftshift(out_ifft)

    def _get_gaussian_window(width, height, std=3.14, mode=0):
        window_scale_x = width / min(width, height)
        window_scale_y = height / min(width, height)
        x = (np.arange(width) / width * 2. - 1.) * window_scale_x
        fy = np.outer(np.arange(height) / height * 2. - 1., np.ones(width))
        if mode == 0:
            window = np.exp(-(x ** 2 + fy ** 2) * std)
        else:
            window = (1 / ((x ** 2 + 1.) * (fy ** 2 + 1.))) ** (std / 3.14)
        return window

    def _get_masked_window_rgb(np_mask_grey, hardness=1.):
        hardened = np_mask_grey ** hardness
        np_mask_rgb = np.tile(hardened[:, :, np.newaxis], (1, 1, 3))
        return np_mask_rgb

    width, height, num_channels = _np_src_image.shape
    ...
    noise_rgb += (1. - color_variation) * noise_grey[:, :, np.newaxis]

    noise_fft = _fft2(noise_rgb)
    noise_fft *= noise_window[:, :, np.newaxis]
    noise_rgb = np.real(_ifft2(noise_fft))
    shaped_noise_fft = _fft2(noise_rgb)
    shaped_noise_fft *= (src_dist ** noise_q) * src_phase
    ...
    matched_noise = shaped_noise.copy()
