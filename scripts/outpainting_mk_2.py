...
class Script(scripts.Script):
    ...
    def run(self, p, _, pixels, mask_blur, direction, noise_q, color_variation):
        ...
        def expand(
            ...
            img.paste(
                init, (pixels_horiz if is_left else 0, pixels_vert if is_top else 0)
            )
            ...
            target_width = (
                min(process_width, init.width + pixels_horiz) if is_horiz else img.width
            )
            target_height = (
                min(process_height, init.height + pixels_vert)
                if is_vert
                else img.height
            )
            ...
            out.paste(
                proc_img,
                (
                    0 if is_left else out.width - proc_img.width,
                    0 if is_top else out.height - proc_img.height,
                ),
            )
