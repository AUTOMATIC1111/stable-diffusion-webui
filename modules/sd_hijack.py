...
def apply_optimizations():
    ...
    if cmd_opts.force_enable_xformers or (cmd_opts.xformers and shared.xformers_available and torch.version.cuda and (6, 0) <= torch.cuda.get_device_capability(shared.device) <= (9, 0)):
        ...
        print('Warning: specified --xformers but it is not available.')

    if cmd_opts.opt_sub_quad_attention:
        ldm.modules.attention.CrossAttention.forward = sd_hijack_optimizations.sub_quad_attention_forward
        ldm.modules.diffusionmodules.model.AttnBlock.forward = sd_hijack_optimizations.sub_quad_attnblock_forward
        methods.append('sub-quadratic')
    ...
    if len(methods) > 0:
        ...
        if len(methods) > 1:
            print('Warning: multiple optimization methods specified: %s, defaulting to: %s' % (methods, optimization_method))
