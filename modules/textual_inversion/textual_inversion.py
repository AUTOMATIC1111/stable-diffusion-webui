...
class EmbeddingDatabase:
    ...
    def read_embedding_from_image(self, path, name):
        try:
            ...
            if (
                cache_embedding := self.image_embedding_cache.get(path)
            ) and ondisk_mtime == cache_embedding.get("mtime", 0):
                # cache will only be used if the file has not been modified time matches
                return cache_embedding.get("data", None), cache_embedding.get(
                    "name", None
                )
            ...
            if data is None or shared.opts.textual_inversion_image_embedding_data_cache:
                ...
                self.image_embedding_cache[path] = {
                    "data": data,
                    "name": None if data is None else name,
                    "mtime": ondisk_mtime,
                }
