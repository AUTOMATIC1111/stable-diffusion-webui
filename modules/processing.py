...
class StableDiffusionProcessing():
    """
    The first set of paramaters: sd_models -> do_not_reload_embeddings represent the minimum required to create a
...
class Processed:
    def __init__(self, p: StableDiffusionProcessing, images_list, seed=-1, info="", subseed=None, all_prompts=None,
        ...
        self.subseed = int(self.subseed if type(self.subseed) != list else self.subseed[0]
...
def create_infotext(p, all_prompts, all_seeds, all_subseeds, comments, iteration=0, position_in_batch=0):
    ...
    generation_params = {
        ...
        "Model hash": getattr(p, 'sd_model_hash',
            None if not opts.add_model_hash_to_info or not shared.sd_model.sd_model_hash else
            shared.sd_model.sd_model_hash
        ),
        "Model": (
            None if not opts.add_model_name_to_info or not shared.sd_model.sd_checkpoint_info.model_name else
            shared.sd_model.sd_checkpoint_info.model_name.replace(
                ',', ''
            ).replace(':', '')),
        "Hypernet": (None if shared.loaded_hypernetwork is None else shared.loaded_hypernetwork.name),
        "Hypernet strength": (
            None if shared.loaded_hypernetwork is None or shared.opts.sd_hypernetwork_strength >= 1 else
            shared.opts.sd_hypernetwork_strength),
        "Batch size": (None if p.batch_size < 2 else p.batch_size),
        "Batch pos": (None if p.batch_size < 2 else position_in_batch),
        "Variation seed": (None if p.subseed_strength == 0 else all_subseeds[index]),
        "Variation seed strength": (None if p.subseed_strength == 0 else p.subseed_strength),
        "Seed resize from": (
            None if p.seed_resize_from_w == 0 or p.seed_resize_from_h == 0 else f"{p.seed_resize_from_w}x"
                                                                                f"{p.seed_resize_from_h}"),
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    try:
        for k, v in p.override_settings.items():
            ...
            # since it is relatively fast to load on-change, while SD models are not
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    print(f"process_images_inner: {p}")
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def __init__(self, enable_hr: bool = False, denoising_strength: float = 0.75, firstphase_width: int = 0,
            firstphase_height: int = 0, **kwargs
    ):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        samples = self.sampler.sample(self, x, conditioning, unconditional_conditioning,
            image_conditioning=self.txt2img_image_conditioning(x, self.firstphase_width,
                self.firstphase_height
            )
        )
        ...
        """saves image before applying hires fix, if enabled in options; takes as an arguyment either an image or 
        batch with latent space images"""
        ...
        samples = self.sampler.sample_img2img(self, samples, noise, conditioning, unconditional_conditioning,
            steps=self.steps, image_conditioning=image_conditioning
        )
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def __init__(self, init_images: list = None, resize_mode: int = 0, denoising_strength: float = 0.75,
            mask: Any = None,
            mask_blur: int = 4, inpainting_fill: int = 0, inpaint_full_res: bool = True,
            inpaint_full_res_padding: int = 0, inpainting_mask_invert: int = 0, **kwargs
    ):
        super().__init__(**kwargs)
