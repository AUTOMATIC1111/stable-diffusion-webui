...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    if type(p.seed) == list:
        ...
        all_seeds = [
            ...
            for x in range(len(all_prompts))
        ]
    ...
    with torch.no_grad(), precision_scope("cuda"), ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            uc = prompt_parser.get_learned_conditioning(
                len(prompts) * [p.negative_prompt], p.steps
            )
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def __init__(
        self, enable_hr=False, scale_latent=True, denoising_strength=0.75, **kwargs
    ):
    ...
    def sample(
        ...
        samples = samples[
            :,
            :,
            truncate_y // 2 : samples.shape[2] - truncate_y // 2,
        ...
        if self.scale_latent:
            ...
            if opts.upscaler_for_img2img is None or opts.upscaler_for_img2img == "None":
                decoded_samples = torch.nn.functional.interpolate(
                    decoded_samples, size=(self.height, self.width), mode="bilinear"
                )
            else:
