...
parser.add_argument("--config", type=str, default=os.path.join(script_path,
                    "configs/v1-inference.yaml"), help="path to config which constructs model",)
...
parser.add_argument("--clip-models-path", type=str,
                    help="Path to directory with CLIP model file(s).", default=None)
...
parser.add_argument("--freeze-settings", action='store_true',
                    help="disable editing settings", default=False)
...
parser.add_argument("--api-auth", type=str,
                    help='Set authentication for API like "username:password"; or comma-delimit multiple like "u1:p1,u2:p2,u3:p3"', default=None)
parser.add_argument("--api-log", action='store_true',
...
parser.add_argument("--ui-debug-mode", action='store_true',
                    help="Don't load model to quickly launch UI")
...
parser.add_argument("--administrator", action='store_true',
                    help="Administrator rights", default=False)
...
parser.add_argument("--tls-keyfile", type=str,
                    help="Partially enables TLS, requires --tls-certfile to fully function", default=None)
parser.add_argument("--tls-certfile", type=str,
                    help="Partially enables TLS, requires --tls-keyfile to fully function", default=None)
parser.add_argument("--server-name", type=str,
                    help="Sets hostname of server", default=None)
parser.add_argument("--img-output-dir", type=str,
                    help="Sets the output directory for images", default=None)
...
class State:
    ...
    def do_set_current_image(self):
        ...
        if opts.show_progress_grid:
            self.current_image = modules.sd_samplers.samples_to_image_grid(
                self.current_latent)
        else:
            self.current_image = modules.sd_samplers.sample_to_image(
                self.current_latent)
...
def listfiles(dirname):
    filenames = [os.path.join(dirname, x) for x in sorted(
        os.listdir(dirname)) if not x.startswith(".")]
