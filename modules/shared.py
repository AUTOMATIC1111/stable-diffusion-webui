...
parser.add_argument(
    ...
    default=os.path.join(model_path, "Codeformer"),
)
parser.add_argument(
    "--gfpgan-models-path",
    type=str,
    help="Path to directory with GFPGAN model file(s).",
    default=os.path.join(model_path, "GFPGAN"),
)
parser.add_argument(
    "--esrgan-models-path",
    type=str,
    help="Path to directory with ESRGAN model file(s).",
    default=os.path.join(model_path, "ESRGAN"),
)
parser.add_argument(
    "--bsrgan-models-path",
    type=str,
    help="Path to directory with BSRGAN model file(s).",
    default=os.path.join(model_path, "BSRGAN"),
)
parser.add_argument(
    "--realesrgan-models-path",
    type=str,
    help="Path to directory with RealESRGAN model file(s).",
    default=os.path.join(model_path, "RealESRGAN"),
)
parser.add_argument(
    "--scunet-models-path",
    type=str,
    help="Path to directory with ScuNET model file(s).",
    default=os.path.join(model_path, "ScuNET"),
)
parser.add_argument(
    "--swinir-models-path",
    type=str,
    help="Path to directory with SwinIR model file(s).",
    default=os.path.join(model_path, "SwinIR"),
)
parser.add_argument(
    "--ldsr-models-path",
    type=str,
    help="Path to directory with LDSR model file(s).",
    default=os.path.join(model_path, "LDSR"),
)
parser.add_argument(
    "--opt-split-attention",
    action="store_true",
    help="force-enables cross-attention layer optimization. By default, it's on for torch.cuda and off for other torch devices.",
)
parser.add_argument(
...
class OptionInfo:
    def __init__(
        self, default=None, label="", component=None, component_args=None, onchange=None
    ):
    ...
    options_section(
        ("system", "System"),
        {
            ...
            "multiple_tqdm": OptionInfo(
                True,
                "Add a second progress bar to the console that shows progress for an entire job. Broken in PyCharm console.",
            ),
        },
    )
)
    ...
    options_section(
        ("sd", "Stable Diffusion"),
        {
            ...
            "enable_emphasis": OptionInfo(
                True,
                "Eemphasis: use (text) to make model pay more attention to text and [text] to make it pay less attention",
            ),
    ...
    options_section(
        ("interrogate", "Interrogate Options"),
        {
            ...
            "interrogate_clip_dict_limit": OptionInfo(
                1500, "Interrogate: maximum number of lines in text file (0 = No limit)"
            ),
        },
    )
)
...
options_templates.update(
    options_section(
        ("ui", "User interface"),
        {
            ...
            "js_modal_lightbox_initialy_zoomed": OptionInfo(
