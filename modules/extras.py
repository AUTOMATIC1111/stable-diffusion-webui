...
def run_extras(extras_mode, image, image_folder, gfpgan_visibility, codeformer_visibility, codeformer_weight, upscaling_resize, extras_upscaler_1, extras_upscaler_2, extras_upscaler_2_visibility):
    ...
    for image, image_name in zip(imageArr, imageNameArr):
        if image is None:
            return [pi.get_filename() for pi in outputs], "Please select an input image.", ''
        ...
        pi = ProcessedImage(image, infotext=None, infotext_extras=(existing_pnginfo, info))
    ...
    return [pi.get_filename() for pi in outputs], plaintext_to_html(info), ''
