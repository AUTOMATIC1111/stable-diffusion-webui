...
#   Call Discord Bot to post picture    #
    try:
        post_result(str(fullfn),prompt,str(seed),str(p.subseed),str(p.subseed_strength),sd_samplers.samplers[p.sampler_index].name,str(p.steps),str(p.cfg_scale),str(p.width),str(p.height))
    except:
        pass
#########################################
