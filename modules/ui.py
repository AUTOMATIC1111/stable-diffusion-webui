...
def save_files(js_data, images, do_make_zip, index):
    ...
    if index > -1 and opts.save_selected_only and (index >= data[
        "index_of_first_image"]):  # ensures we are looking at a specific non-grid picture, and we have
        # save_selected_only
...
def check_progress_call(id_part):
    ...
    if opts.show_progressbar:
        progressbar = f"""<div class='progressDiv'><div class='progress' style="overflow:visible;width:
        {progress * 100}%;white-space:nowrap;">
        {"&nbsp;" * 2 + str(int(progress * 100)) + "%" + time_left if progress > 0.01 else ""}</div></div>"""
    ...
    return f"<span id='{id_part}_progress_span' style='display: none'>{time.time()}</span><p>{progressbar}</p>", \
...
def create_toprow(is_img2img):
    ...
    return prompt, roll, prompt_style, negative_prompt, prompt_style2, submit, button_interrogate, button_deepbooru, \
        prompt_style_apply, save_style, paste, token_counter, token_button
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        txt2img_prompt, roll, txt2img_prompt_style, txt2img_negative_prompt, txt2img_prompt_style2, submit, _, _, \
            txt2img_prompt_style_apply, txt2img_save_style, txt2img_paste, token_counter, token_button = create_toprow(
            is_img2img=False
        )
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='panel'):
                ...
                seed, reuse_seed, subseed, reuse_subseed, subseed_strength, seed_resize_from_h, seed_resize_from_w, \
                    seed_checkbox = create_seed_inputs()
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        img2img_prompt, roll, img2img_prompt_style, img2img_negative_prompt, img2img_prompt_style2, submit, \
            ...
            token_counter, token_button = create_toprow(
            is_img2img=True
        )
            ...
            with gr.Column(variant='panel'):
                with gr.Tabs(elem_id="mode_img2img") as tabs_img2img_mode:
                    with gr.TabItem('img2img', id='img2img'):
                        init_img = gr.Image(label="Image for img2img", elem_id="img2img_image", show_label=False,
                            source="upload", interactive=True, type="pil", tool=cmd_opts.gradio_img2img_tool
                        ).style(height=480)

                    with gr.TabItem('Inpaint', id='inpaint'):
                        init_img_with_mask = gr.Image(label="Image for inpainting with mask", show_label=False,
                            elem_id="img2maskimg", source="upload", interactive=True, type="pil",
                            tool=cmd_opts.gradio_inpaint_tool, image_mode="RGBA"
                        ).style(height=480)
                        ...
                        if use_color_sketch:
                            ...
                            init_img_with_mask.change(update_orig, [init_img_with_mask, init_img_with_mask_orig],
                                init_img_with_mask_orig
                            )

                        init_img_inpaint = gr.Image(label="Image for img2img", show_label=False, source="upload",
                            interactive=True, type="pil", visible=False, elem_id="img_inpaint_base"
                        )
                        ...
                        with gr.Row():
                            ...
                            mask_alpha = gr.Slider(label="Mask transparency", interactive=use_color_sketch,
                                visible=use_color_sketch
                            )
                        ...
                        with gr.Row():
                            ...
                            inpainting_mask_invert = gr.Radio(label='Masking mode', show_label=False,
                                choices=['Inpaint masked', 'Inpaint not masked'], value='Inpaint masked', type="index"
                            )

                        inpainting_fill = gr.Radio(label='Masked content',
                            choices=['fill', 'original', 'latent noise', 'latent nothing'], value='original',
                            type="index"
                        )
                    ...
                    with gr.TabItem('Batch img2img', id='batch'):
                        hidden = '<br>Disabled when launched with --hide-ui-dir-config.' if \
                            shared.cmd_opts.hide_ui_dir_config else ''
                        gr.HTML(
                            f"<p class=\"text-gray-500\">Process images in a directory on the same machine where the "
                            f"server is running.<br>Use an empty output directory to save pictures normally instead "
                            f"of writing to the output directory.{hidden}</p>"
                        )
                ...
                with gr.Row():
                    resize_mode = gr.Radio(label="Resize mode", elem_id="resize_mode", show_label=False,
                        choices=["Just resize", "Crop and resize", "Resize and fill"], type="index", value="Just resize"
                    )
            ...
            with gr.Column(variant='panel'):
                ...
                with gr.Group():
                    extras_upscaler_1 = gr.Radio(label='Upscaler 1', elem_id="extras_upscaler_1",
                        choices=[x.name for x in shared.sd_upscalers], value=shared.sd_upscalers[0].name, type="index"
                    )
                ...
                with gr.Group():
                    codeformer_visibility = gr.Slider(minimum=0.0, maximum=1.0, step=0.001,
                        label="CodeFormer visibility", value=0, interactive=modules.codeformer_model.have_codeformer
                    )
    ...
    with gr.Blocks(analytics_enabled=False) as modelmerger_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='panel'):
                gr.HTML(
                    value="<p>A merger of the two checkpoints will be generated in your <b>checkpoint</b> "
                          "directory.</p>"
                )
    ...
    with gr.Blocks(analytics_enabled=False) as train_interface:
        with gr.Row().style(equal_height=False):
            gr.HTML(
                value="<p style='margin-bottom: 0.7em'>See <b><a "
                      "href=\"https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/Textual-Inversion\">wiki"
                      "</a></b> for detailed explanation.</p>"
            )
            ...
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Create hypernetwork"):
                    ...
                    new_hypernetwork_initialization_option = gr.Dropdown(value="Normal",
                        label="Select Layer weights initialization. Recommended: Kaiming for relu-like, Xavier for "
                              "sigmoid-like, Normal otherwise",
                ...
                with gr.Tab(label="Train"):
                    gr.HTML(
                        value="<p style='margin-bottom: 0.7em'>Train an embedding or Hypernetwork; you must specify a "
                              "directory with a set of 1:1 ratio images <a "
                              "href=\"https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/Textual-Inversion"
                              "\" style=\"font-weight:bold;\">[wiki]</a></p>"
                    )
                    with gr.Row():
                    ...
                    log_directory = gr.Textbox(label='Log directory',
                        placeholder="Path to directory where to write outputs", value="textual_inversion"
                    )
                    ...
                    with gr.Row():
                        ...
                        tag_drop_out = gr.Slider(minimum=0, maximum=1, step=0.1,
                            label="Drop out tags when creating prompts.", value=0
                        )
                    with gr.Row():
    ...
    def run_settings(*args):
        ...
        for key, value, comp in zip(opts.data_labels.keys(), args, components):
            assert comp == dummy_component or opts.same_type(value, opts.data_labels[key].default
    ...
    def loadsave(path, x):
        def apply_field(obj, field, condition=None, init_field=None):
            ...
            if saved_value is None:
                ...
                print(
                    f'Warning: Bad ui setting value: {key}: {saved_value}; Default value "{getattr(obj, field)}" will '
                    f'be used instead.'
                )
            else:
