...
def update_token_counter(text, steps, multicond=True):
    try:
        if multicond:
            _, prompt_flat_list, _ = prompt_parser.get_multicond_prompt_list([text])
            prompt_schedules = prompt_parser.get_learned_conditioning_prompt_schedules(prompt_flat_list, steps)
        else:
            prompt_schedules = prompt_parser.get_learned_conditioning_prompt_schedules([text], steps)
...
def create_toprow(is_img2img):
                ...
                with gr.Column(scale=80):
                    with gr.Row():
                        prompt = gr.Textbox(label="Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=2, 
                            placeholder="Prompt (Alt+Enter for quick Generating)\n"
                        )

                with gr.Column(scale=1, elem_id="token_counter_col"):
                ...
                with gr.Column(scale=8):
                    with gr.Row():
                        negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"{id_part}_neg_prompt", show_label=False, lines=2, 
                            placeholder="Negative prompt (Alt+Enter for quick Generating)"
                        )

                with gr.Column(scale=1, elem_id="token_counter_col"):
                    neg_token_counter = gr.HTML(value="<span></span>", elem_id=f"{id_part}_neg_token_counter")
                    neg_token_button = gr.Button(visible=False, elem_id=f"{id_part}_neg_token_button")
    ...
    return prompt, roll, prompt_style, negative_prompt, prompt_style2, submit, interrogate, deepbooru, prompt_style_apply, save_style, paste, token_counter, token_button, neg_token_counter, neg_token_button
...
def create_ui(wrap_gradio_gpu_call):
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        txt2img_prompt, roll, txt2img_prompt_style, txt2img_negative_prompt, txt2img_prompt_style2, submit, _, _, txt2img_prompt_style_apply, txt2img_save_style, paste, token_counter, token_button, neg_token_counter, neg_token_button = create_toprow(is_img2img=False)
        ...
        with gr.Row().style(equal_height=False):
            ...
            neg_token_button.click(fn=lambda text, steps: update_token_counter(text, steps, False), inputs=[txt2img_negative_prompt, steps], outputs=[neg_token_counter])

    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        img2img_prompt, roll, img2img_prompt_style, img2img_negative_prompt, img2img_prompt_style2, submit, img2img_interrogate, img2img_deepbooru, img2img_prompt_style_apply, img2img_save_style, paste, token_counter, token_button, neg_token_counter, neg_token_button  = create_toprow(is_img2img=True)
        ...
        with gr.Row().style(equal_height=False):
            ...
            neg_token_button.click(fn=lambda text, steps: update_token_counter(text, steps, False), inputs=[img2img_negative_prompt, steps], outputs=[neg_token_counter])
