...
def save_files(js_data, images, index):
    ...
    if save_to_dirs:
        dirname = apply_filename_pattern(
            opts.directories_filename_pattern or "[prompt_words]", p, p.seed, p.prompt
        )
    ...
    with open(
        ...
        truncated = (
            (file_decoration[:240] + "..")
            if len(file_decoration) > 240
            else file_decoration
        )
...
def wrap_gradio_call(func, extra_outputs=None):
    def f(*args, extra_outputs_array=extra_outputs, **kwargs):
        ...
        res[
            -1
        ] += f"<div class='performance'><p class='time'>Time taken: <wbr>{elapsed:.2f}s</p>{vram_html}</div>"
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                ...
                with gr.Row():
                    batch_count = gr.Slider(
                        minimum=1,
                        maximum=cmd_opts.max_batch_count,
                        step=1,
                        label="Batch count",
                        value=1,
                    )
            ...
            modules.generation_parameters_copypaste.connect_paste(
                paste, txt2img_paste_fields, txt2img_prompt
            )
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                ...
                with gr.Tabs(elem_id="mode_img2img") as tabs_img2img_mode:
                    ...
                    with gr.TabItem("Batch img2img", id="batch"):
                        ...
                        gr.HTML(
                            f'<p class="text-gray-500">Process images in a directory on the same machine where the server is running.{hidden}</p>'
                        )
            ...
            save.click(
                fn=wrap_gradio_call(save_files),
                _js="(x, y, z) => [x, y, selected_gallery_index()]",
                inputs=[generation_info, img2img_gallery, html_info],
                outputs=[
                    html_info,
                    html_info,
                    html_info,
            ...
            for button, (prompt, negative_prompt), (style1, style2) in zip(
            ...
            modules.generation_parameters_copypaste.connect_paste(
                paste, img2img_paste_fields, img2img_prompt
            )
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                ...
                interp_method = gr.Radio(
                    choices=["Weighted Sum", "Sigmoid", "Inverse Sigmoid"],
                    value="Weighted Sum",
    ...
    with gr.Blocks() as textual_inversion_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column():
                with gr.Group():
                    gr.HTML(
                        value="<p style='margin-bottom: 0.7em'>Create a new embedding</p>"
                    )
        ...
        with gr.Row(elem_id="settings").style(equal_height=False):
            for i, (k, item) in enumerate(opts.data_labels.items()):
                ...
                if previous_section != item.section:
                    ...
                    gr.HTML(
                        elem_id="settings_header_text_{}".format(item.section[0]),
                        value='<h1 class="gr-button-lg">{}</h1>'.format(
                            item.section[1]
                        ),
                    )
