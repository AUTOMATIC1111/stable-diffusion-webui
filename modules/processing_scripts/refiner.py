...
class ScriptRefiner(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        with InputAccordion(True, label="Refiner", elem_id=self.elem_id("enable")) as enable_refiner:
            with gr.Row():


                # Searching for a refiner to display by default in the following dropdown (refiner_checkpoint)
                string_refiner = ''
                for elt in sd_models.checkpoint_tiles():
                    if elt.__contains__('refiner'):
                        string_refiner = elt
                

                refiner_checkpoint = gr.Dropdown(label='Checkpoint - Pick None to disable any checkpoint', elem_id=self.elem_id("checkpoint"), choices=sd_models.checkpoint_tiles(), value=string_refiner, tooltip="switch to another model in the middle of generation")
