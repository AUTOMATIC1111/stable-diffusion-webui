...
group.add_argument("--ui-config", type=str, default=os.environ.get("SD_UI-CONFIG", os.path.join(data_path, 'ui-config.json')), help="Use specific UI configuration file, default: %(default)s")
...
group.add_argument("--models-dir", type=str, default=os.environ.get("SD_MODELDIR", 'models'), help="Base path where all models are stored, default: %(default)s",)
group.add_argument("--allow-code", default=os.environ.get("SD_ALLOW_CODE", False), action='store_true', help="Allow custom script execution, default: %(default)s")
