...
parser.add_argument("--vram-profile", type=str, choices=["default", "balanced", "fast", "ultra"], default="default", help="select a built-in VRAM profile; 'ultra' aggressively lowers memory usage, 'fast' prioritizes throughput")
parser.add_argument("--vram-optimization-mode", type=str, choices=["balanced", "saver", "ultra"], default="balanced", help="reduce VRAM pressure by cleaning unused memory more aggressively; 'ultra' is best for low-memory GPUs")
parser.add_argument("--vram-adaptive-batch-limit", type=int, default=0, help="limit batch size automatically when memory saver modes are active; 0 disables adaptive batching")
parser.add_argument("--safe-mode", action='store_true', help="enable the safest low-memory generation profile with aggressive cleanup and minimal batch size")
