...
def get_tagger(
    ...
    add_confident_as_weight: bool, # 将置信度作为权重写入生成的Tags (不推荐勾选)
    replace_underscore: bool, # 使用空格替代下划线 (推荐勾选)
    replace_underscore_excludes: str, # 排除项(逗号分隔)
    escape_tag: bool, # 转义括号(防止误识别为权重信息，推荐勾选)
    unload_model_after_running: bool): # 完成后从显存中卸载模型 (推荐勾选)
    
    tags_res={}
    # 判断反推模型存在
    interrogator='wd14-vit-v2-git'
    if interrogator not in interrogators:
        raise OSError(f'cannot found interrogator:{interrogator}')
    ...
    for path in images:
        try:
            ...
            print(f'${image} is not supported image type')
            continue
...
class TaggerTask:
    ...
    def exec_task(cls, task: Task):
        ...
        # 改成Tageer反推
        return get_tagger(t.images, t.interrogator, t.threshold, t.additional_tags, t.exclude_tags, t.sort_by_alphabetical_order,
