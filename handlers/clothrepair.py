...
from handlers.utils import init_script_args, get_selectable_script, init_default_script_args, \
    load_sd_model_weights, save_processed_images, get_tmp_local_path, get_model_local_path, save_processed_images_shanshu
...
def clothes_predict(input_image):
    ...
    negative_points = [[358,324], [150, 485], [540, 467], [290, 710], [418, 710], [309, 243], [421, 247], [90, 177], [600, 130], [295, 642], [425,642]]
    text_prompt = None
    dino_enabled = text_prompt is not None
    boxes_filt = None
    box_threshold = 0.3
...
def get_hoodie_image(imageA, imageB, coord1, coord2, coord3, coord4, coord5, coord6, coord7, coord8, coord9, coord10, angle, clothes_type):
    ...
    res_img = resize_and_overlay_image(imageA, rotated_areaC, coord5[0], coord5[1], coord6[0] - coord5[0], coord6[1] - coord5[1])
    res_img = resize_and_overlay_image(res_img, rotated_areaD, coord7[0], coord7[1], coord8[0] - coord7[0], coord8[1] - coord7[1])
    res_img = resize_and_overlay_image(res_img, imageB_core, coord9[0], coord9[1], coord10[0] - coord9[0], coord10[1] - coord9[1])
...
def get_coord(stable_area, clothes_type, frontor_back):
    ...
    if clothes_type=="cardigan":
        if frontor_back=="正面":
            ...
            coord5, coord6, coord7, coord8, coord9, coord10 =  (130, 327), (273, 610), (447, 327), (590, 610), (243, 326), (467, 636)
...
class ClothesRepairTaskHandler(Txt2ImgTaskHandler):
    ...
    def _exec_ClothesRepair(self, task: Task) -> typing.Iterable[TaskProgress]:
        ...
        progress = TaskProgress.new_ready(full_task, 'clothes_repair')
        yield progress
        ...
        if full_task.clothes_type == "shirt":
            ...
            coord1, coord2, coord3, coord4, coord5, coord6, coord7, coord8, coord9, coord10, coord11, coord12 = get_coord(stable_area, full_task.clothes_type, full_task.frontor_back)
        ...
        if full_task.is_psed != True:
            ...
            if full_task.clothes_type == "hoodie" or full_task.clothes_type == "cardigan":
                if full_task.clothes_type == "hoodie":
                    angle = 60
                    fusion_image = get_hoodie_image(template_image, clothes_img, coord1, coord2, coord3, coord4, coord5, coord6,
        ...
        # 第二部分，重绘服装边缘，生图
        processed,process_args = inpaint_clothes(image_dic, template_image, full_task.text_prompt, full_task.batch_size)
