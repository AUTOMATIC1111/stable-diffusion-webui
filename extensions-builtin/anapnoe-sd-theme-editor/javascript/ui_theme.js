...
function hexToRgb(color) {
  ...
  if (hex.length !== 6) {
    hex = function () {
      ...
      for (var _i = 0, _Array$from = Array.from(hex); _i < _Array$from.length; _i++) {
        c = _Array$from[_i];
...
function rgbToHsl(rgb) {
  ...
  var diff = max - min;
  var add = max + min;
  var hue = min === max ? 0 : r === max ? (60 * (g - b) / diff + 360) % 360 : g === max ? 60 * (b - r) / diff + 120 : 60 * (r - g) / diff + 240;
  var lum = 0.5 * add;
  var sat = lum === 0 ? 0 : lum === 1 ? 1 : lum <= 0.5 ? diff / add : diff / (2 - add);
...
function hslToHex(h, s, l) {
  ...
  var f = function f(n) {
    ...
    return Math.round(255 * Math.max(0, Math.min(color, 1))).toString(16).padStart(2, "0"); // convert to Hex and prefix "0" if needed
  };
  ...
  for (var key in styleobj) {
    ...
    if (keyVal.indexOf("#") != -1 || keyVal.indexOf("hsl") != -1) {
      ...
      if (colcomp) {
        ...
        var hex = hslToHex(h, Math.min(Math.max(s, 0), 100), Math.min(Math.max(l, 0), 100));
        colcomp.value = hex;
...
function initTheme(styles) {
  ...
  var intervalChange;
  document.body.querySelectorAll("#ui_theme_settings input").forEach(function (elem) {
  ...
  reset_btn.addEventListener("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    document.body.querySelectorAll("#ui_theme_hsv input").forEach(function (elem) {
      elem.value = 0;
    });
    hsloffset = [0, 0, 0];
  ...
  var hsloffset = [0, 0, 0];
  var hue = document.body.querySelectorAll("#theme_hue input").forEach(function (elem) {
  ...
  var sat = document.body.querySelectorAll("#theme_sat input").forEach(function (elem) {
    elem.addEventListener("change", function (e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  var brt = document.body.querySelectorAll("#theme_brt input").forEach(function (elem) {
    elem.addEventListener("change", function (e) {
      e.preventDefault();
      e.stopPropagation();
