...
class BlueprintGenerator:
    ...
    def generate(self, user_config: dict, argparse_namespace: argparse.Namespace, **runtime_params) -> Blueprint:
        ...
        argparse_config = {optname_map.get(optname, optname): value for optname, value in
                           vars(sanitized_argparse_namespace).items()}
        ...
        for dataset_config in sanitized_user_config.get("datasets", []):
            ...
            for subset_config in subsets:
                params = self.generate_params_by_fallbacks(subset_params_klass,
                                                           [subset_config, dataset_config, general_config,
                                                            argparse_config, runtime_params])
                subset_blueprints.append(SubsetBlueprint(params))

            params = self.generate_params_by_fallbacks(dataset_params_klass,
                                                       [dataset_config, general_config, argparse_config,
                                                        runtime_params])
...
def generate_dreambooth_subsets_config_by_args(list_repeats, list_class_tokens, list_train_data_dir, list_reg_tokens=[],
                                               list_reg_data_dir=[]):
    subsets_config = []
    for i in range(len(list_train_data_dir)):
        n = int(list_repeats[i])
        dir = list_train_data_dir[i]
        subset_config = {"image_dir": str(dir), "num_repeats": n, "is_reg": False, "class_tokens": list_class_tokens[i]}
        subsets_config.append(subset_config)
    if list_reg_data_dir != [] and list_reg_data_dir is not None:
        for i in range(len(list_train_data_dir)):
            dir = list_train_data_dir[i]
            n = int(list_repeats[i])
            subset_config = {"image_dir": str(dir), "num_repeats": n, "is_reg": True,
                             "class_tokens": list_reg_tokens[i]}
            subsets_config.append(subset_config)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    train_util.add_dataset_arguments(parser, config_args.support_dreambooth, config_args.support_finetuning,
