...
def generate_dreambooth_subsets_config_by_args(list_repeats, list_class_tokens, list_train_data_dir, list_reg_repeats=[],list_reg_tokens=[],list_reg_data_dir=[]):
  subsets_config = []
  ...
  if list_reg_data_dir!=[] and list_reg_data_dir is not None:
    for i in range(len(list_reg_data_dir)):
      n = int(list_reg_repeats[i])
      dir = list_reg_data_dir[i]
      subset_config = {"image_dir": str(dir), "num_repeats": n, "is_reg": True,"class_tokens": list_reg_tokens[i]}
      subsets_config.append(subset_config)
